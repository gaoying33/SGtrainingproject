package com.sgcc.uap.app.controller;
...
import com.sgcc.uap.app.services.ICustomerService;
import com.sgcc.uap.app.vo.CustomerVO;
...
public class CustomerController {
	...
	public WrappedResult getById(@PathVariable String id) {
		try {
			QueryResultObject result = customerService.getCustomerById(id);
			logger.info("查询成功"); 
	...
	public WrappedResult deleteByIds(@RequestBody IDRequestObject idObject) {
		try {
			...
			customerService.remove(idObject);
	...
	public WrappedResult saveOrUpdate(@RequestBody FormRequestObject<Map> params) {
		try {
			...
			result.setFormItems(customerService.saveCustomer(map));
			logger.info("保存数据成功"); 
			return WrappedResult.successWrapedResult(result);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	public WrappedResult query(@QueryRequestParam("params") RequestCondition requestCondition) {
		...
		try {
			QueryResultObject queryResult = customerService.query(requestCondition);
			logger.info("查询数据成功"); 
	...
	public 	WrappedResult getMetaData(
		...
		try{
			...
			datas = ViewAttributeUtils.getViewAttributes(columns, CustomerVO.class);
